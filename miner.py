...
class PDFMiner:
    def __init__(self, filepath):
        ...
        # finding the nearest available width value in zoomdict
        available_widths = sorted(zoomdict.keys(), reverse=True)
        for available_width in available_widths:
            if width >= available_width:
                self.zoom = zoomdict[available_width]
                break
        else:
            # if no available width found, use default zoom value
            self.zoom = 1.0
